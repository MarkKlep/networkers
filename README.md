...
This starts all nine processes — six `blog/` microservices, `auth`, `referrals`, and `tutor` — with labeled, color-coded output, and a single Ctrl+C stops all of them. Each subproject still needs its own dependencies installed once first (see [`CLAUDE.md`](./CLAUDE.md) for per-service setup, including `blog/query`'s Python venv and `blog/client`'s `.env` for Google sign-in). `npm run stop` frees every port the stack uses if a crashed run leaves one held.
...
| [`tutor`](./tutor) | Express (Node) | 4008 | Practice's AI coach — hints while you solve, and a review of each submission. Proxies to a local Ollama model, so nothing leaves your machine |
